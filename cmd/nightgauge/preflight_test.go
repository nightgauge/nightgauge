...
func TestPreflightCmd_RegistersAllSubcommands(t *testing.T) {
	...
	sort.Strings(got)
	want := []string{"ac-reconcile <issue-number>", "dependency-guard", "links", "secrets", "skill-anti-patterns", "skill-includes", "skill-no-direct-gh", "skill-portability", "syntax", "thinking-effort"}
	if len(got) != len(want) {
