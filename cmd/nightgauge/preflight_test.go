...
func TestPreflightCmd_RegistersAllSubcommands(t *testing.T) {
	...
	sort.Strings(got)
	want := []string{"ac-reconcile <issue-number>", "dependency-guard", "links", "managed-steering", "mitigation-rule", "platform-raw-http", "secrets", "skill-anti-patterns", "skill-includes", "skill-no-direct-gh", "skill-portability", "skill-shell-state", "syntax", "thinking-effort"}
	if len(got) != len(want) {
