...
var packageLiteralAllowlist = map[string]bool{
	...
	" | ":      true,
	" ":        true,
	// The one word character glued to a `~term`'s literal — at the LEFT edge and
	// at the RIGHT edge, as two separate derived inputs, because the boundary is
	// a conjunction of two independent tests — to derive the inputs plain
	// containment claims and the word boundary must not. Mirrored verbatim in the
	// TypeScript twin as BOUNDARY_VIOLATION_AFFIX.
	"s": true,
...
func TestStressInputsAreDerivedFromTheTable(t *testing.T) {
	...
	if len(inputs) < 1000 {
		t.Fatalf("only %d stress inputs — the derivation has been narrowed; it must cover every "+
			"clause, every term, both edges of every `~` term, every ordered rule pair and every "+
			"signal rule composed with every extension clause", len(inputs))
	}
	...
	// THE TWO BLOCKS THAT WERE MISSING, ASSERTED STRUCTURALLY. A count floor
	// cannot see a whole group of derived inputs disappear, and "the derivation
	// is blind to the mechanism" is how both round-5 findings happened: the
	// mechanism existed, no input sampled it, and deleting it moved nothing. A
	// golden diff is not enough on its own — a golden can be regenerated.
	for _, group := range [][][]string{ruleClausesOf(tb), extensionClausesOf(tb)} {
		for _, clause := range group {
			for i, term := range clause {
				if !strings.HasPrefix(term, WordBoundaryRef) {
					continue
				}
				for _, atLeftEdge := range []bool{false, true} {
					probe := tb.sampleClauseViolatingBoundary(clause, i, atLeftEdge)
					if seen[probe] {
						continue
					}
					t.Errorf("no stress input violates the word boundary of %q in clause %v on the "+
						"%s edge (expected %q).\n"+
						"containsWordBounded is a CONJUNCTION of two tests; an unsampled edge is a "+
						"conjunct anyone can delete in one line with every artifact byte-identical.",
						term, clause, edgeName(atLeftEdge), probe)
				}
			}
		}
	}
	...
	for _, r := range tb.Rules {
		...
		for _, e := range tb.SignalExtensions {
			for _, clause := range e.Clauses {
				...
				for _, probe := range []string{rule + " | " + extension, extension + " | " + rule} {
					if seen[probe] {
						continue
					}
					t.Errorf("no stress input composes signal rule %q with clause %v of extension "+
						"%q (expected %q).\n"+
						"Without an input carrying BOTH, the stage order of SignalKind — the only "+
						"thing that makes \"an extension can never overrule a `signal: true` rule\" "+
						"true — is enforced by nothing.", r.ID, clause, e.ID, probe)
				}
			}
		}
	}
...
func ruleClausesOf(tb *Table) [][]string {
	var out [][]string
	for _, r := range tb.Rules {
...
func extensionClausesOf(tb *Table) [][]string {
	var out [][]string
	for _, e := range tb.SignalExtensions {
...
func edgeName(atLeftEdge bool) string {
	if atLeftEdge {
		return "left"
	}
	return "right"
