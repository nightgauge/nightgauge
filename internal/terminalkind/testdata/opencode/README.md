...
| Model servers   | `127.0.0.1` only: the stub provider (#1618), a one-status server, or an empty `ollama serve` |
...
| `ollama-model-not-pulled` | `ollama serve`, `-m ollama/qwen3:8b`, not pulled: 404             | `AI_APICallError: model 'qwen3:8b' not found`                | `APIError`             |
...
text to classify. Ollama is: `ollama-model-not-pulled` is a real `ollama serve`
(0.32.11) on a loopback port, with a throwaway `HOME` and an empty model
directory, asked for a model it has not pulled. The leg is skipped when
`ollama` is not on `PATH`. Its message is the chat handler's
`model '<name>' not found`; Ollama's scheduler words the same failure
`model "<name>" not found, try pulling it first`
([`server/routes.go`](https://github.com/ollama/ollama/blob/v0.32.11/server/routes.go)
`handleScheduleError`), which the corpus pins through the same line shape.
...
model the loopback LM Studio lacks is not an error. A model Ollama has not
pulled is one, in Ollama's words inside OpenCode's wrapper. The
classification rules follow what was observed.
...
(and `ollama` for the Ollama leg), takes about three minutes (the down-server
and 500 legs wait out OpenCode's retries), and writes nothing unless every
file passes the credential scan. Raising the version re-captures this
directory in the same change as ADR-022 § 20's other captures.
