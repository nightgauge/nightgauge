...
## OpenCode: `opencode_stream_expected.json`

The expectations both OpenCode parsers are held to (#1637): the Go parser
(`TestOpenCodeSharedExpectations` in `stream_test.go`) and the SDK's
(`packages/nightgauge-sdk/src/cli/adapters/__tests__/opencodeStream.test.ts`)
read this one file, so the two languages assert one set of numbers. It is
written by hand from the ground-truth tables above, not generated by either
parser, and it holds no capture data of its own: every number in it is one of
those tables' rows or sums.

Each entry is keyed by a capture's file name, and both tests fail when an
`opencode_*.jsonl` capture has no entry. An entry holds:

- `dispatched`: the `-m` value the capture ran with.
- `stream`: what the parser reads from the capture alone. The session, the
  `step_finish` count, the summed tokens with reasoning kept apart from output
  (the Go accumulator folds it in, so the Go test compares output plus
  reasoning), the peak step prompt, OpenCode's own reported cost, and the tool
  calls OpenCode rejected.
- `stderr` and `failures`: the capture's stderr, and the marker it ends a
...
- `fold`: for a capture whose session exports were recorded at capture time,
  ...
  it, and the stage's cost under ADR-022 § 3, where `null` means unstamped.
  The cloud stand-in's cost is the registry's rate card for `grok-4.6` (0.34
  USD per million input tokens, 1.02 per million output), not OpenCode's
  0.006262.
