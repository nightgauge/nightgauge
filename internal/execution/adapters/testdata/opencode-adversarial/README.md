...
`@REMOTE_URL@` in `q3-array-merge/remote-project/opencode.json` becomes the
loopback model stub's URL.
...
| 3   | Do `plugin`, `instructions` and `mcp` concatenate with the inline values or get replaced?      | `TestOpenCodeArrayMerge`                   | `instructions` and `plugin` concatenate, the project's first. `mcp` merges by server name, the inline entry winning for a name both set (its `command` replaces, not appends). An empty inline list removes nothing (KNOWN): `debug config` then reports `plugin: []`, but `plugin_origins` still lists the project's plugin and it loads. A project `instructions` URL is fetched (KNOWN): with an empty inline list, the run requests it from the loopback stub the URL names (`q3-array-merge/remote-project/`) before its model request.                                                                                                                                                                |
...
- Empty the `instructions` list of `q3-array-merge/remote-project/opencode.json`:
  the stub sees no instructions request and `TestOpenCodeArrayMerge` fails.
