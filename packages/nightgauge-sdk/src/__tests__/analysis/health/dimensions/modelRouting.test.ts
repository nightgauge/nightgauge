...
import type { HealthAnalysisInput } from "../../../../analysis/health/types.js";
import {
 ...
 * Spreads issueNumber so records within one batch are uniquely identifiable.
 */
...
function makeEmptyDataset(): HealthAnalysisInput {
  return datasetFromRecords([]);
}

function makeDataset(): HealthAnalysisInput {
  return datasetFromRecords([
    ...buildRecords(5, {
      model: "haiku",
      selectionSource: "scheduler",
...
describe("analyzeModelRouting — under-routing", () => {
  ...
  it("detects under-routing for a concrete haiku-band model ID", () => {
    ...
    expect(result.metrics["underRoutingCount"]).toBe(5);
    expect(result.findings.some((finding) => finding.title.includes("Under-Routing"))).toBe(true);
  });
...
describe("analyzeModelRouting — over-routing", () => {
  ...
  it("detects over-routing for a concrete opus-band model ID", () => {
    ...
    expect(result.metrics["overRoutingCount"]).toBe(5);
    expect(result.findings.some((finding) => finding.title.includes("Over-Routing"))).toBe(true);
  });
...
// ── 9. Combined penalty stacking ───────────────────────────────────

describe("analyzeModelRouting — combined penalty stacking", () => {
  it("stacks the under-routing, over-routing, and low-accuracy penalties", () => {
    // Combined penalties: under-routing, over-routing, and low accuracy.
...
// ── 12. The vocabulary this dimension actually filters on (#446) ────
...
describe("analyzeModelRouting — mixed local and cloud fleets (#2055)", () => {
  it("does not flag a cloud model as expensive because local records cost zero", () => {
    const records = [
      ...buildRecords(5, {
        model: "mistral",
        costUsd: 0,
    ...
    expect(result.findings.find((finding) => finding.title.includes("Cost-Ineffective"))).toBe(
      undefined
    );
  });

  it("computes mean effective cost from cloud models only", () => {
    const records = [
      ...buildRecords(5, {
        model: "llama-3",
        costUsd: 0,
  ...
  it("reports an all-local fleet without a cost-inefficiency finding", () => {
    const records = buildRecords(10, {
      model: "mistral",
      costUsd: 0,
      success: true,
...
describe("modelRouting test ownership", () => {
  it("has no shadow copy in the non-typechecked tests root", () => {
    const shadowCopy = resolve(
      __dirname,
      "../../../../../tests/analysis/health/dimensions/modelRouting.test.ts"
    );

    expect(existsSync(shadowCopy)).toBe(false);
  });
});
