...
export const TERMINAL_KIND_TABLE: TerminalKindTable = {
  ...
  "rules": [
    ...
    {
      "id": "dev-step-progress-unproven",
      "kind": "dev_step_progress_unproven",
      "signal": false,
      "clauses": [
        [
          "[dev-step-progress-unproven]"
        ],
        [
          "dev_step_progress_unproven"
        ]
      ],
      "why": "The feature-dev step loop (#1651) could not fingerprint the work tree (git failed on it) around a sub-session that checked no plan task, so it cannot tell a step that changed files from one that changed nothing; it stops rather than count the step as progress. Environment class: the worktree is the fault, not the model or the issue, and the earlier steps' work is on disk. It must not read as dev_produced_no_changes, which it has not shown, or as subagent_crash, which git's own `exit status` text would otherwise match. Its only producer is the step loop's own marker."
    },
    {
    ...
    },
    {
      ...
      "why": "The adapter rejected, by itself, a tool the stage's allowed tools GRANT (#1624, #1631): opencode resolved the permission to `ask`, auto-rejected it headless, and the parser (internal/execution/opencode_usage.go) ended the stage's stderr with `[adapter-permission-rejected] tool=\u003cpermission\u003e`. Nightgauge does not generate an OpenCode permission map yet (#1638), so the `ask` comes from OpenCode's own defaults or from a repository's or the user's opencode.json. OpenCode 1.18.30's defaults ask before `read` on `*.env` and `*.env.*` (allowing `*.env.example`), so a stage allowed Read that reaches for a secret file, on its own or because the issue text asked it to, ends `[adapter-permission-rejected] tool=read`; that parks like every other permission, because a retry would let the model or the issue text loop the issue against a guard that must not be loosened. The next attempt runs under the same rule and is rejected identically, so the kind is parked after one dispatch, not retried, and not charged to the issue. The permission the marker names never changes the kind: it is the model's tool choice. MUST precede permission-denied: that rule is the harness saying `not that way` to a tool the stage was NOT allowed, which a short backoff and a different approach can clear. The parser gives one stage one marker, but a reason can join text from more than one source (a stage's error and its tail, a composed first-cause reason), and where both appear the configuration defect is the one no retry clears, so it names the failure. The marker, not the notice, is matched: the notice's patterns are the model's own tool input, which the parser strips before anything is kept (ADR-022 § 9)."
    },
    {
