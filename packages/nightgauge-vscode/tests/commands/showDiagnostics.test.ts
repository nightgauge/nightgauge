...
// the test fails.
// Both are ASSEMBLED AT RUNTIME rather than written as literals. A verbatim
// JWT in the tree trips the `jwt` rule in the credential scan (which walks the
// full history, so a later fix cannot un-commit it), and a fixture that is
// indistinguishable from a committed secret is a bad fixture regardless. The
// assembled values are still real-shaped at runtime, which is the only part
// redactSecrets has to catch.
const FAKE_SESSION_JWT = [
  Buffer.from(JSON.stringify({ alg: "HS256", typ: "JWT" })).toString("base64url"),
  Buffer.from(JSON.stringify({ sub: "1234567890", iat: 1700000000 })).toString("base64url"),
  "s".repeat(43),
].join(".");
