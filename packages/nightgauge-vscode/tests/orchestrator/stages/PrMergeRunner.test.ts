/**
 * PrMergeRunner unit tests
 *
 * Verifies that the pr-merge runner:
 * - Returns failure when pr-{N}.json prerequisite is missing
...
import { PrMergeRunner } from "../../../src/orchestrator/stages/PrMergeRunner";
...
const PREREQ_PATH = "/workspace/.nightgauge/pipeline/pr-42.json";

function makeSuccessResult(): StageRunResult {
  return { success: true, stage: "pr-merge", durationMs: 10 };
}
...
describe("PrMergeRunner", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe("when prerequisite pr-{N}.json exists", () => {
    beforeEach(() => {
      vi.mocked(fs.existsSync).mockReturnValue(true);
    });

    it("delegates to ctx.executeSkill", async () => {
      const runner = new PrMergeRunner();
      const ctx = makeCtx();
  ...
  describe("when prerequisite pr-{N}.json is missing", () => {
    beforeEach(() => {
    ...
    it("returns an error describing the missing prerequisite", async () => {
      ...
      expect(result.error?.message).toContain("pr-merge");
    ...
    it("includes the correct stage in the failure result", async () => {
      ...
      expect(result.stage).toBe("pr-merge");
    });
    ...
    it("checks the correct context path for the prerequisite", async () => {
      ...
      expect(ctx.getContextPath).toHaveBeenCalledWith("pr", 42);
    });
  });
});
