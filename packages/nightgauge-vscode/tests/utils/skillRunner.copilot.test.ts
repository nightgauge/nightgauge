...
// after the test has returned, which used to race vitest's worker RPC
// teardown (`EnvironmentTeardownError: Closing rpc while "onUserConsoleLog"
// was pending`). That race is now closed at the source: every reachable
// `console.log`/`warn`/`error` call site in skillRunner.ts is routed through
// a vitest-guarded diagnostic shim (Issue #173), so no per-file console spy
// is needed here.
