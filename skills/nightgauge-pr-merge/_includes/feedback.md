# Phase 7.8: Retrospective Feedback — Procedural Detail

This file holds the step-by-step procedure for Phase 7.8 (Retrospective
Feedback) of the `nightgauge-pr-merge` skill: detect execution mode,
prepare and prompt retrospective questions, parse/validate responses, and
persist feedback. Every step here is **non-blocking** — no step calls `exit 1`.
...
- [Step RF.6: Write Assessment Record (Optional)](#step-rf6-write-assessment-record-optional)
- [Error Handling](#error-handling)

**PURPOSE**: Capture post-merge feedback on the workflow for continuous skill
improvement. This phase is **non-blocking** — errors are logged but never fail
the stage. In headless mode the prompt is skipped gracefully.
...
All steps in this phase are **non-blocking**. No step should call `exit 1`.
