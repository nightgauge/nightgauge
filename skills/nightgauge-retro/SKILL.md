...
# `issue list --search` is scoped to open issues
EXISTING=$(nightgauge issue list --repo "$REPO" --search "${SEARCH_TITLE}" --limit 5 --json | jq '. // []')
...
*Auto-created by /nightgauge:retro --epic ${EPIC_NUMBER} --create-issues*"
    ...
    # Check for duplicate
    EXISTING=$(nightgauge issue list --repo "$REPO" --search "\"${PATTERN}\" label:skill-drift" --json 2>/dev/null \
      | jq -r '(. // [])[0].number // empty')
    if [ -n "$EXISTING" ]; then
      ...
      if [ "$DRY_RUN" = "true" ]; then
        ...
        nightgauge issue create --repo "$REPO" --title "$TITLE" --body "$BODY" --labels "skill-drift,priority:medium" 2>/dev/null || \
