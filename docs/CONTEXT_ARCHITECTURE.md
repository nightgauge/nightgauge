...
| `dev.build_verification.recorded_as_prose`                                             | quarantined | `build_verification` is absent and `quality_checks.build` holds the build outcome as free text; reading "succeeded" out of prose into `{ran, status}` is inference, so the object is named untrustworthy and `FeatureDevGate` derives it from git as `unverified` (#1482). |
