...
| `scheduled_tasks.*` (release_watch, docs_watch, continuous_improvement)                         | team YAML          | Team        | The _whether it runs_ — team policy. The cadence itself lives in the workflow cron, not here.                                                |
...
| `github_auth.*`                                                    | schema only                           | Machine     | Org-to-user fallback mappings — multi-identity workspaces, per developer. `token` and `tokens.<owner>` are refused in the project and local tiers unless they are `env:` references.                                                                                                                                                                                                                                                             |
...
| `lm_studio.*` (model, context_length, base_url, stream_options, …) | removed (#2128)                       | Machine     | The `lm-studio` adapter and its block were removed (#2128). Declare the server under `opencode.endpoints`.                                                                                                                                                                                                                                                                                                                                       |
| `ollama.*`                                                         | removed (#2128)                       | Machine     | The `ollama` adapter and its block were removed (#2128). Declare the server under `opencode.endpoints`.                                                                                                                                                                                                                                                                                                                                          |
...
| `platform.*` (api keys, telemetry, retry policy)                   | schema only                           | Machine     | Cloud-platform credentials and per-user telemetry opt-out. Stripped from the project and local tiers; a plaintext `license_key` there is refused.                                                                                                                                                                                                                                                                                                |
...
| `lm_studio.*`                                   | team YAML                       | Machine | Removed (#2128): the block no longer exists; declare the server under `opencode.endpoints`.                                                                                                                                                     |
| `ui.core.adapter` (team YAML)                   | team YAML                       | Machine | Copy `ui.core.adapter` → machine YAML; offer cleanup of the team YAML key. (The `nightgauge.core.adapter` VSCode setting was deleted in #968 — it reached no code, so there is no VSCode side to migrate to.)                                   |
...
- **OQ-7: `contextPath` and `plansPath` — team or machine?**
