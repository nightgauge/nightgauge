...
| C3  | `scripts/post-merge-check.sh` green at the release commit (#2055: the merged PR's gate on the same tree, plus CodeQL on `main`)    | **Done** — `v0.2.2` is `1ac50c6f`; that commit's own run on `main`: 18 success, 2 skipped, 0 failed (re-read 2026-09-02).                                      |
...
| L6  | `scripts/check-changelog.sh --tag vX.Y.Z` exits 0: the root `CHANGELOG.md` **and** the extension changelog both have the section | **Re-opened and closed 2026-09-05** — the 0.2.x rows above were true only of the extension file; the root changelog had never been sectioned. Both now are, and `release.yml` runs this check before publishing. |
...
| **The weekly G1 gate now runs in CI, but its agent credential is invalid.** `CLEAN_INSTALL_GH_TOKEN` (fine-grained, scoped to the throwaway `nightgauge-e2e` organization via `CLEAN_INSTALL_E2E_OWNER`) is set and proven: six dispatches on 2026-09-02 walked install → board → pickup → `feature-planning`. The last one (run 33694698201) failed there in 7 s with `Invalid API key`: the `ANTHROPIC_API_KEY` repository secret (set 2026-08-20) is rejected by the API. Replace it, then dispatch `clean-install-e2e.yml` once. | Only the owner can mint a key. Everything the six runs found on the way is fixed: image never built on amd64 (#1325), hidden run directory never uploaded (#1326), Projects V2 transient conflict (#1328), extension logs not collected (#1330), fine-grained tokens refused by the auth pre-check (#1331); #1329 tracks the missing failure detail. |
