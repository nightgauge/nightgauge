...
| **`CLEAN_INSTALL_GH_TOKEN` is not set**, so the weekly `clean-install-e2e.yml` gate skips every Monday and G1 is only ever walked from a maintainer's machine.                                                                                                                                                                                    | Create a dedicated throwaway organization, set the `CLEAN_INSTALL_E2E_OWNER` variable to it, and mint a fine-grained token whose resource owner is **that organization only** (Administration, Contents, Issues, Pull requests, Projects: read/write). Never a token with Administration write over the real organization. |
