...
| C3  | `scripts/post-merge-check.sh` green at the release commit (#2055: the merged PR's gate on the same tree)                           | **Done** — `v0.2.2` is `1ac50c6f`; that commit's own run on `main`: 18 success, 2 skipped, 0 failed (re-read 2026-09-02).                                      |
