...
| workflow-gate: block force push to main | stdin | `{"tool_name":"Bash",...}`         | Blocked when `hooks.push_gate` protects main |
| workflow-gate: block push main          | stdin | `{"tool_name":"Bash",...}`         | Blocked when `hooks.push_gate` protects main |
