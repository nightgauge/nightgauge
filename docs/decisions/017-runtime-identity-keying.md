...
| `RuntimeState.ClaimTerminal` (steps 1b–1d)    | `rs.mu` (once)                               | — (it **is** the composite)             | **no** — caller holds `runtimesMu` only    |
...
`ClaimTerminal` is the one exception, and the row above says so: it is the
**sanctioned exported composite** for claim steps 1b–1d, called by
`internal/ipc` while that package holds `runtimesMu` and no `rs.mu`. It is sound
for the reason C16's rule is really about — it ACQUIRES `rs.mu` once from a
caller that holds none, so there is no re-entry and no cycle. **`rs.mu` being
unexported to package `ipc` is exactly why the composite has to live in the
state layer**: the claim's three mutations are declared atomic, and a caller
that cannot hold the mutex cannot make them atomic itself. Composing them here
is the only way the normative sentence is literally true rather than aspirational.
...
**The honest caveat: "microseconds" describes the claim's own work, not the
worst case.** `ClaimTerminal` waits for `rs.mu`, and a concurrent `Persist` on
the same run holds `rs.mu` across a marshal AND an fsync'ed atomic write (which
is itself deliberate — F27). So `runtimesMu` is transitively held for the
duration of that write whenever the two collide: milliseconds, and every other
run's resolution queues behind it. This is **accepted**, and it is a LIVENESS
cost rather than a correctness one — nothing is dropped, reordered or
mis-attributed, callers simply wait. The alternative (release `runtimesMu`
before taking `rs.mu`) reopens the window an ordinary transition uses to append
a stage after the replay was declared the run's last accepted mutation, which is
a correctness defect. Trading a rare millisecond of contention for that is not a
close call either. If the contention ever shows up in practice, the fix is to
shorten `Persist`'s hold — not to widen the claim's.
...
singleton's raw notify sites, and its whole traffic profile is ONE `running`
transition: a conversation emits no token stream, so nothing refreshes arm 1 and
nothing re-persists, which ages arm 4 out of the same silence. Thirty minutes
into a session, arms 1, 2 and 4 are false and arm 5 expired at 120 seconds — so
the pid is not a fallback for this population, it is the only arm it has. The
command therefore defers its single `running` transition until
`runStageSkillInteractive` returns and sends the child's pid with it, exactly as
`runStage.ts` and `HeadlessOrchestrator` do on the headless path
(`utils/skillRunner.ts` surfaces it through the same `onStageChildSpawned`
callback). Sent pid-less — which is what it did before this — a live interactive
session past the window is closed as an orphan.

**The Codex TUI sub-path cannot be given arm 3 from this host, and that is a
verified API limit rather than an omission.** #4024 launches the Codex TUI with
`vscode.window.createTerminal` + `sendText`, so the extension host never holds a
`ChildProcess`; VSCode exposes no pid for a process running _inside_ a terminal
(`Terminal.processId` is documented as "the process ID of the **shell**
process"; `TerminalShellExecution` carries only `commandLine`, `cwd` and
`read()`). The shell's pid is not a substitute: it exists before the stage
starts and outlives it whenever the launch command's trailing `exit` does not
run, so it would answer arm 3 for a session that ended — a pin that never
expires, which is the one thing 7.4's rows cannot correct for. This sub-path is
therefore carried by arms 1 and 4 only, and a >30-minute silent TUI session past
the grace is reconcilable. A liveness producer for it (terminal-close driven,
not pid driven) is follow-up work.
...
because 7.1's entire premise is that it is still alive. An `abandoned: true`
snapshot with **no** `abandoned_at` (unreachable from `markAbandonedLocked`,
reachable by hand-authored JSON) is unageable, so it is treated as fresh — but
it falls through to the age cap rather than returning, or "unageable" would mean
"immortal", which is the exemption the paused row already had to lose.

**"Canonical is never overwritten" is structural, not a check.**
`os.Stat`-then-`os.Rename` is check-then-act, and `rename(2)` replaces its
destination silently: a claimant suspended past the release threshold that wakes
and re-persists **between** the two calls loses its live snapshot to pre-pause
content — F34's consequence (1) through a race instead of through the aging
rule. The release therefore uses `os.Link(artifact, canonical)` + `os.Remove`,
which fails atomically on an existing destination (POSIX `EEXIST`, Windows
`ERROR_ALREADY_EXISTS`) and so **is** the no-replace rename this row describes.
On a filesystem with no hardlink support the code falls back to stat+rename and
says so in the log, so the weaker guarantee is visible rather than inferred.
...
| `HeadlessOrchestrator` direct entry points                                                                                             | receive the slot's id when manager-driven; `beginRun` at their own entry otherwise. **Exporter gap, recorded not papered over:** step 0b wires `NIGHTGAUGE_RUN_ID` only at `SkillRunner.ts:344`, the Go-scheduler spawn; the other four `runStageSkillHeadless` call sites (`HeadlessOrchestrator.ts:13261`, `bootstrap/services.ts:3298`, `commands/runStage.ts:855`, `ContextAssembler.ts:595`) hold no identity to export until `beginRun` exists, and gain the export in step 3 from that installed identity. |
...
| `TestOrphanReconcile_AResurrectedRunSurvivesTheReapItRacesWith`                                                               | A run silent past `LIVENESS_WINDOW` that sends one `notifyStageProgress` from **inside** the reap's unlocked phase keeps its entry and its snapshot. The compare-and-delete sees entry _replacement_ only, and `touchLocked` refreshes `LastSeen` in place, so the final locked phase re-applies the age predicate. Verified failing against the pointer-only compare.                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `TestOrphanReconcile_InteractiveShapedRunSurvivesOnItsStagePidAlone`                                                          | An interactive-shaped run — ONE `running` transition through the real handler, then silence, lease and mtime both aged out, grace expired, both registries otherwise cold — is neither reaped nor emitted for nor removed **because its `stagePid` names a live process**; the same run with no pid on the wire is closed. Pins the interactive population's only arm (C18).                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `TestOrphanReconcile_SilentRowsCollectWithoutEmitting`                                                                        | A nil-analytics pass over a terminal snapshot, a stale abandonment and a stale claim removes all three and builds **no** terminal event for any of them — each run was already closed by someone else, and a second `pipeline_done` would double-book it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `TestOrphanReconcile_TheLaunchRootIsScannedAfterASwitch`                                                                      | A `workspace.setRoot` inside the startup grace does not narrow the deferred pass: the launch root is immutable and always scanned, so its orphans are not lost by the only sweep this process performs.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `TestRunIdentity_AdministrativeInstallDoesNotRescueItsSnapshot`                                                               | The 7.3 clause step 4 deferred: after an administrative verb installs an entry for an unknown id, `skipRun` is **false** for that run's snapshot and a post-grace pass emits for it and removes it. Verified failing against a lease-blind arm 1.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `TestRunIdentity_ClaimSequenceIsDeadlockFree`                                                                                 | The whole of Decision 5 executed literally, with a watchdog: N concurrent `notifyComplete`s, run-progress calls, derived-index scans and reconciler passes over one repo, under `-race`, with a hard timeout that **fails the test rather than hanging CI**. A build-tag-guarded variant asserts that the claim path calls no exported `RuntimeState` method **other than `ClaimTerminal`** between `runtimesMu.Lock` and its `Unlock` — the original wording, read literally, forbids the shipped and correct form, whose whole point is that the composite is the ONE exported method the claim may call there (see the lock-discipline table). Verified failing against a literal transcription of the second revision's sequence, which wedges on the first call. Covers **F36** and C16. |
...
4. **The server re-keys, and the verbs require — one transaction, and the
   ...
   **verb-class-first resolution with the per-class × per-registry policy**
   ...
   | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | `pipeline.notifyStageTransition` | scheduler `IpcStageRunner` (0b); extension `PipelineStateService` ×7 raw sites (3) — the ×7 only assert when their DRIVER installed an identity, so the drivers are enumerated here too. COVERED IN 3: `HeadlessOrchestrator.{runPipeline,runStage,startNextQueuedIssue}`, `ConcurrentPipelineManager.startSlotInner`, `commands/retryFailedIssue`, `commands/runStage` and `commands/runInteractiveStage` (all receive-or-mint with a keyed release). UNREACHABLE, wired but dead: `bootstrap/services.ts` `contextWatcher.onIssuePickedUp` — the watcher is `suspend()`ed unconditionally at bootstrap (#1831) and nothing calls `resume()`/`scanExistingContext()`, so this population cannot emit until somebody revives it. RESOLVED BY DELETION (#427): `StaleSlotRecoveryService` built a fresh `PipelineStateService.createForWorktree(worktreePath)` with no issue number and no run to name and called `failStage` — a REPAIR, not a dispatch — and step 4 was expected to grow a global/administrative arm for it. It never needed one: the service scanned a `<worktree>/.nightgauge/pipeline/state.json` that nothing writes, so the emitter could not fire in production, and the condition it claimed (extension host dies mid-run) is the reconciler's own — §7.2 arm 3. Both halves were deleted rather than given an identity |
   | `pipeline.notifyPhaseTransition` | scheduler `PipelineBridge:294` — sends `runId: ipcParams.runId`, non-empty by the step-0b dispatch assertion; the server-side param field lands in step 2, until when `encoding/json` discards it (0b); extension `PipelineStateService.{startPhase:1076,completePhase:1120}` — both fire-and-forget with silent catches, so their assertion must land with the beginRun installation (3)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
   | `pipeline.notifyStageProgress`   | scheduler `PipelineBridge:372` — same: value source in 0b, param field in 2 (0b); extension `HeadlessOrchestrator:13323` and `bootstrap/services.ts:3094` (3)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
   | `pipeline.notifyComplete`        | extension `PipelineStateService.notifyPipelineComplete:921` (3)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
   | `pipeline.setPaused`             | extension `PipelineStateService.{pause,resume}Pipeline` (3) — **AND a second emitter, discovered in step 3:** `MattermostCommandDispatcher.ts` global pause/resume calls `pipelineSetPaused(0, …)` with issue number 0 and no run to name, because it pauses the WHOLE pipeline rather than a run. It can never supply an identity, so step 4 must either keep a global arm for `issueNumber == 0` or reroute that command before flipping this verb                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
   | `pipeline.abandonRun`            | its only caller is the force-clear funnel, which lands **with the verb** in step 6                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |

   **`setPaused` has an emitter that can never assert, and step 3 found it.**
   `MattermostCommandDispatcher`'s `/pause` and `/resume` send
   `pipelineSetPaused(0, …)` — a GLOBAL pause with issue number 0, naming no
   run because there is no run to name. Step 3 left that code untouched rather
   than inventing an identity for it, so this verb cannot be flipped to
   `run_id_required` unconditionally: step 4 must keep a global arm for
   `issueNumber == 0` (an operator-wide pause is a different transaction from
   pausing one run) or reroute the command onto a verb that means what it says.
