...
#               running, a required context has not appeared, the merged tree
#               was never tested, or the API could not be read. Wait and re-run
#
...
#      default-branch baseline, cache-warm) must be green; still running is
#      NOT-YET. An empty list is NOT-YET for five minutes after the merge (the
#      push workflows may not exist yet) and passes after that.
#
...
# no longer run on push that reads NOT-YET, because nothing tested the landed
# tree; not RED, because nothing failed. A commit with no merged PR (a direct
# push) is judged by that same merge-commit-only rule.
#
...
# --- Fallback: the binary could not be resolved. Same rule as the binary
# (github.EvaluateMergedCommit), minus the per-run cross-check, and with the
# required-check set read from the base branch's rulesets and (best-effort)
...
# check-run shape, pending ones as still running. An API failure is not
...
if [[ -z "$pr" ]]; then
  ...
  judge_commit "$SHA" "$merge_checks" null
fi
...
# The base branch's required contexts. Rulesets are readable with repository
# read access; classic protection needs more and is best-effort. If neither
# can be read the set is unknown (null), and the PR head is judged on every
# check it carries, which is stricter, never looser.
REQ=null
if rules=$(gh api --paginate "repos/$REPO/rules/branches/$BASE_REF" \
  ...
  classic=$(gh api "repos/$REPO/branches/$BASE_REF/protection/required_status_checks" --jq '.contexts[]' 2>/dev/null) || classic=""
...
# no longer run on push that reads NOT-YET, not GREEN: nothing has tested the
# landed tree. It is not RED either, because RED means a check failed.
...
# runs on the merge commit (CodeQL, cache-warm) must be green or still running.
...
if [[ "$merge_total" -eq 0 ]]; then
  age=$(jq -n --arg t "$MERGED_AT" 'now - ($t | fromdateiso8601) | floor' 2>/dev/null || echo 0)
