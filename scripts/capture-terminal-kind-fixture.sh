...
#
# Issue #306: terminal-kind classification is defined ONCE, as the ordered rule
# table at internal/terminalkind/table.json, which Go embeds and the SDK reads
# through a generated module. The shared corpus in
# internal/terminalkind/testdata/corpus.json says what those rules must mean for
# concrete failure text, and every corpus row marked `"source": "captured"` must
# appear in the file THIS script emits — captured-shapes.json — which is how
# #166's evidence rule stops being a claim and starts being a test.
#
...
# the canonical rule table, in table order, so the miner automatically follows
# the classifier when the table grows a pattern.
#
# ------------------------------------------------------------------------
...
OUT_DIR="$REPO_ROOT/internal/terminalkind/testdata"
...
OUT_DIR="$OUT_DIR" TABLE_JSON="$REPO_ROOT/internal/terminalkind/table.json" \
  python3 - "$@" <<'PY'
import glob
import json
import os
...
#
# In table order (rules, then clauses, then terms), deduped, predicate
# references skipped — they are code, not text to mine for. This used to parse
# `strings.Contains(t, "…")` out of the Go source; the table made that
# unnecessary and the ordering identical, because the table IS the source order.
...
payload = {
    "$comment": [
        ...
        "internal/terminalkind/corpus_test.go, so a hand-authored string",
    ...
    "markers_in_rule_table": len(vocabulary),
    "shapes": [
        {
            "text": r["text"],
            "origin": r["origin"],
